...
### Recovered Figure 1 — Post-Surgical Outcomes by Patient
...
ggplot(patients_df, aes(x = patient, y = post_surgical_score, 
  ...
  # Mean lines - thicker and more visible
  geom_hline(yintercept = mean_dreamy, color = "#4E79A7", linetype = "dashed", linewidth = 1.5, alpha = 0.8) +
  geom_hline(yintercept = mean_duck, color = "#E15759", linetype = "dashed", linewidth = 1.5, alpha = 0.8) +
  # Mean annotations - larger text with label box on the right
  annotate("label", x = max(patients_df$patient) * 0.95, y = mean_dreamy, 
           label = paste("○", round(mean_dreamy, 2)), 
           color = "#4E79A7", fill = "white", alpha = 0.8, hjust = 1, vjust = -0.5, 
           size = 5, fontface = "bold", label.padding = unit(0.3, "lines")) +
  annotate("label", x = max(patients_df$patient) * 0.95, y = mean_duck, 
           label = paste("△", round(mean_duck, 2)), 
           color = "#E15759", fill = "white", alpha = 0.8, hjust = 1, vjust = -0.5, 
           size = 5, fontface = "bold", label.padding = unit(0.3, "lines")) +
  # Colorblind-friendly colors and shapes
  scale_color_manual(name = "Doctor",
                     values = c("Doc Dreamy" = "#4E79A7", "Doc Duck" = "#E15759"),
                     labels = c("Doc Dreamy", "Doc Duck")) +
  scale_shape_manual(name = "Doctor",
...
### Recovered Figure 2 — Randomized Assignment Outcomes
...
ggplot(patients_randomized_df, aes(x = patient, y = post_surgical_score, 
  ...
  geom_point(size = 2.5, alpha = 0.7) +
  # Mean lines - thicker and more visible
  geom_hline(yintercept = mean_dreamy_rand, color = "#4E79A7", linetype = "dashed", linewidth = 1.5, alpha = 0.8) +
  geom_hline(yintercept = mean_duck_rand, color = "#E15759", linetype = "dashed", linewidth = 1.5, alpha = 0.8) +
  # Mean annotations - larger text with label box on the right
  annotate("label", x = max(patients_randomized_df$patient) * 0.95, y = mean_dreamy_rand, 
           label = paste("○", round(mean_dreamy_rand, 2)), 
           color = "#4E79A7", fill = "white", alpha = 0.8, hjust = 1, vjust = -0.5, 
           size = 5, fontface = "bold", label.padding = unit(0.3, "lines")) +
  annotate("label", x = max(patients_randomized_df$patient) * 0.95, y = mean_duck_rand, 
...
### Recovered Figure 3 — Outcomes by Patient Severity (Stratification)
...
ggplot(patients_df, aes(x = severity, y = post_surgical_score, 
                        color = doctor_name, shape = doctor_name)) +
  # Shaded region from -1 to 1
  annotate("rect", xmin = -1, xmax = 1, ymin = -Inf, ymax = Inf, 
           alpha = 0.15, fill = "gray", color = NA) +
  # Scatter points
  geom_point(size = 2.5, alpha = 0.7) +
  # Mean lines - thicker and more visible
  geom_hline(yintercept = mean_dreamy_sev, color = "#4E79A7", linetype = "dashed", linewidth = 1.5, alpha = 0.8) +
  geom_hline(yintercept = mean_duck_sev, color = "#E15759", linetype = "dashed", linewidth = 1.5, alpha = 0.8) +
  # Mean annotations - larger text with label box on the right
  annotate("label", x = max(patients_df$severity) * 0.95, y = mean_dreamy_sev, 
           label = paste("○", round(mean_dreamy_sev, 2)), 
           color = "#4E79A7", fill = "white", alpha = 0.8, hjust = 1, vjust = -0.5, 
           size = 5, fontface = "bold", label.padding = unit(0.3, "lines")) +
  annotate("label", x = max(patients_df$severity) * 0.95, y = mean_duck_sev, 
